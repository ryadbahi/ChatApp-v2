...
const FriendsList: React.FC<FriendsListProps> = ({
  ...
  useEffect(() => {
    ...
    const handleOnlineStatusUpdate = ({
      ...
      setOnlineFriends((prev) => {
        ...
        // Do not call onOnlineFriendsChange here; useEffect will handle it
        return newSet;
      });
    };
    ...
    socket.emit(
      ...
      ({ onlineFriends }: { onlineFriends: User[] }) => {
        // Always set as Set<string> (user IDs)
  ...
  // Notify parent ONLY when onlineFriends changes, not during render or setState
  // Only call when the callback reference is stable (does not change every render)
  useEffect(() => {
    // Only call if the callback is defined and stable
