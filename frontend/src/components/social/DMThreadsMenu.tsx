...
const DMThreadsMenu: React.FC<DMThreadsMenuProps> = ({
  ...
  useEffect(() => {
    ...
    const handleDirectMessagesRead = ({
      ...
      // When I read messages from someone, update that thread's unread count
      setThreads((prev) =>
        prev.map((thread) => {
          // If this thread is from the sender whose messages were just read
          const otherUserId =
            thread.lastMessage?.sender._id === currentUser._id
              ? thread.lastMessage?.recipient._id
    ...
    const handleAllDirectMessagesRead = () => {
      // When someone reads my messages, I don't need to update unread counts
      // This is mainly for updating read status in individual message views
    };
